package com.SpringMongoCollection.Service;
...
import com.SpringMongoCollection.Repository.EmployeeManagerRepository;
...
public class EmployeeService {
    ...
    @Autowired
    private EmployeeManagerRepository employeeManagerRepo;
    ...
    public Response addEmployee(EmployeeManager employee) {
        ...
        Integer maxId = employeeManagerRepo.findMaxId();
        if(maxId != null) {
        ...
        if (employeeManagerRepo.existsById(employee.getId())) {
        ...
        if ("Account Manager".equalsIgnoreCase(employee.getDesignation())) {
            ...
            List<EmployeeManager> existingManagers = employeeManagerRepo.findByDepartment(employee.getDepartment());
            ...
            employeeManagerRepo.insert(employee);
        ...
        EmployeeManager manager = employeeManagerRepo.findById(employee.getManagerId());
        if (manager == null) {
    ...
    public List<ManagerResponse> getEmployee(Integer managerId, Integer yearsOfExperience) {
        ...
        if (managerId != null && yearsOfExperience != null) 
        {
            ...
            employees = employeeManagerRepo.findByManagerIdAndDateOfJoiningBefore(managerId, minJoiningDate);
        } 
        else if (managerId != null) 
        {
            employees = employeeManagerRepo.findByManagerId(managerId);
        } 
        else if (yearsOfExperience != null) 
        {
            
            Instant minJoiningDate =  dateConverter(yearsOfExperience.toString());
            employees = employeeManagerRepo.findByDateOfJoiningBefore(minJoiningDate);
        } 
        else 
        {
            employees = employeeManagerRepo.findAll();
        }
        ...
        List<ManagerResponse> responseList = employees.stream()
                ...
                .map(entry -> {
                    ...
                    EmployeeManager manager = employeeManagerRepo.findById(currentManagerId);
    ...
    public Response changeEmployeeManager(Integer employeeId, Integer newManagerId) {
        // Fetch the employee
        EmployeeManager employee = employeeManagerRepo.findById(employeeId);
        if (employee == null) {
        ...
        EmployeeManager newManager = employeeManagerRepo.findById(newManagerId);
        ...
        // Build the response
        String originalManagerName = employeeManagerRepo.findById(employee.getManagerId()).getName();
        ...
        employeeManagerRepo.save(employee);
